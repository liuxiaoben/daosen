	...
	<div class="sucaizy">
      ...
      <span class="rightButton">
		...
		<div class="sucai-body1">
			<el-row :gutter="15">
				<el-col :span="6" v-for="list in materialList" :key="list">
...
export default {
    ...
    methods: {
        new_sucai() {
            this.$router.push("/enterprise/addMaterial");
        },
        edit(e, materialId) {
            this.$router.push("/enterprise/editMaterial/" + materialId);
        },
        details(e) {
            var materialId = e.target.getAttribute("data-material");
            this.$router.push("/enterprise/materialDetail/" + materialId);
        },
        //获取列表公共方法
        ...
        del: function(e, materialId) {
            mlibDel(this.$store.getters.token, materialId).then(response => {
                if (response.msgCode == "1000") {
                    this.$router.go("/enterprise/material");
                } else {
                    alert("删除失败");
                }
            });
