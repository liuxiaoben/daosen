...
<script>
	...
	export default {
		...
		methods: {
			...
			//选择数量改变将数据加入购物车
			changeMediaNum(cycle, commId, id, num){
				let cart = this.shopCartList;
				let mediaList = this.resourceInfo.children;
				if(!cart[cycle] || !cart[cycle][commId]){//判断购物车是否已经存在该小区媒体
					...
					obj[commId] = {
						...
						commId: this.resourceInfo.commId,
						areaId: this.resourceInfo.area,
						cityId: this.resourceInfo.city,
						provinceId: this.resourceInfo.province,
						province: this.resourceInfo.provinceName,
						city: this.resourceInfo.cityName,
						area: this.resourceInfo.areaName,
						name: this.resourceInfo.commName,
						selTatol: 1,
						priceTatol: 0,
						mediaList: {},
					};
